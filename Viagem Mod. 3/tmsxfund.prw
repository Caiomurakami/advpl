...
@return 	*/
//===========================================================================================================
Static Function VldExistCp( cAliasTab , xConteudo , lMostraErr, nTamKey )
...
If Len(Rtrim(xConteudo)) > nTamKey
	Help("",1,"TMSXFUND01")
	lRet := .F.
Else
...
@version 1.0
-----------------------------------------------------------/*/
Function TMSAgdVgVl(cFilDoc,cDoc,cSerie,dDatViagem,cHorViagem)
									...
									If nPos == 0
										aAdd(aRet,{;
											...
											(	STR0033 + aVeiculo[nY][2] +' '+;   //Tipo de Veiculo:
												STR0026 + DJ4->DJ4_CODRRE +' '+;  //Diferente Do Permitido pela RestriÁ„o:
												STR0032 + (cQryDJ5)->DJ5_ITEM +' '+;  //Item:
												Iif(!Empty((cQryDJ5)->DJ5_CODPRO), STR0028 + Alltrim((cQryDJ5)->DJ5_CODPRO) + ' ' + Alltrim(Posicione("SB1",1,xFilial("SB1") + (cQryDJ5)->DJ5_CODPRO,"B1_DESC")) ,;   //Produto:
												                                   STR0029 + (cQryDJ5)->DJ5_CODGRP + ' ' + Alltrim(Posicione("SBM",1,xFilial("SBM") + (cQryDJ5)->DJ5_CODGRP,"BM_DESC")))+' '+;  //Grupo Prod:
...
If lProcessa
	If !lEstorno  //Inclusao
		...
		lRet:= OMSM011VGE(DTQ->DTQ_FILORI,DTQ->DTQ_VIAGEM,3,lRotAut)
		RestArea(aAreaDTQ)
...
Local cColGFE   := SuperGetMV( "MV_COLGFE" ,.F., "0" )  //0-Padr„o, 1-Encerramento
Local lContinua := .T.
Local lTabDM8   := TableIndic("DM8")
...
If lTMS3GFE .Or. (lTmsRdpU .And. !Empty(cViagem))
	...
	If (cAliasQry)->(!Eof())
		While (cAliasQry)->(!Eof())	
			...
			Else
				lContinua:= .T.
				If (cAliasQry)->DT6_SERIE == 'COL' .And. cColGFE == "1"
					lContinua:= .F.  
				EndIf
			EndIf
			...
			If lContinua
				...
				If (cAliasQry)->DT6_SERIE == 'COL'    //--- Coleta
					...
					If DT5->(MsSeek(xFilial('DT5')+(cAliasQry)->DT6_FILDOC+(cAliasQry)->DT6_DOC+(cAliasQry)->DT6_SERIE))
						...
						If lRet
							...
							If lTabDM8 .And. ExistFunc("TMSFUNGDM8")
				...
				Else
					...
					While (cAliasDOC)->(!Eof())
						...
						If lNumProp
							cEmisDc:= OMSM011COD((cAliasDOC)->DTC_CLIREM,(cAliasDOC)->DTC_LOJREM,1,,)
						Else
	...
	While (cAliasQry)->( !Eof() )
			...
			While (cAliasDTC)->(!Eof())
				...
				If cNota <> (cAliasDTC)->DTC_NUMNFC+(cAliasDTC)->DTC_SERNFC
					...
					If lNumProp
						...
						If !lReentDev
							cDestDc	:= OMSM011COD((cAliasDTC)->DTC_CLIDES,(cAliasDTC)->DTC_LOJDES,1,,)
						Else
							cDestDc	:= OMSM011COD(DTC->DTC_CLIDES,DTC->DTC_LOJDES,1,,)
						EndIf
					Else
		...
		Else
			...
			If DT5->(MsSeek(xFilial('DT5')+ cFilDoc + cDoc + cSerie))
				...
				If Empty(cCliDes)
					cDestDc := OMSM011COD(,,,.T.,DT5->DT5_FILORI)
				...
				Else
					...
					If lNumProp
						cDestDc	:= OMSM011COD(cCliDes,cLojDes,1,,)
					Else
						cDestDc	:= cA1_CGC
					EndIf
				EndIf
...
Function TMSVldVei( cPlanej, cProgram, cViag, dDtIni, cHrIni, dDtFim, cHrFim, aVeic, cViagInt )
	...
	If lMVTMSAloc
		...
		// Valida ProgramaÁ„o
		If lRet .And. ProcName(2) != "TMSA240MNT"
			cAliasQry := GetNextAlias()
...
/*/
//-------------------------------------------------------------------
Function TmGuiaGRNE(nValICMSST, nValICM, nVDifal, nValICMP)
...
Default nValICMP    := 0
			...
			If Empty(IESubTrib(SF2->F2_EST,.T.)) .And. SF2->F2_TIPOCLI=="F" .And. GetMV("MV_ESTADO")<>cUFOrigem
				...
				If lGDifal
					...
					If cUFOrigem $ GetNewPar("MV_SOMAGNR","")
						GravaTit((lGDifal .And. lGFcpDif),nVlrdifal+nDifFecp,"ICMS","IC",cLcPadTit,dDtIni,dDtFim,dDtVenc,1,lGDifal,nMes,nAno,0,nVlrdifal+nDifFecp,"MATA460A",;
								lLancCont,@cNumero,@aGNRE,,,cUFOrigem,,,.T.,,@aRecTit,@lConfTit,0,aDSF2,,,,,,,,,,.T.,,,,.T.,,,(nThreads > 0))
					Else
	...
	If nValICMSST > 0 .And. SF2->F2_TIPO <>"D"  //DLOGTMS02-10141
		Pergunte("TMB200",.F.)
		lLancCont := Iif(ValType(MV_PAR04)<>"N",.F.,(MV_PAR04==1)) //-- Lanc.Contab.On-Line ?
		lTitProd  := Iif(ValType(MV_PAR12)<>"N",.F.,(mv_par12==1)) //-- Gera Titulo Por Produto ? 
		lGRecProd := Iif(ValType(MV_PAR13)<>"N",.F.,(mv_par13==1)) //-- Gera Guia por Produto ?							
...
Local aAreas     := {DM3->(GetArea()),DTR->(GetArea()),DTA->(GetArea()),DUD->(GetArea()),GetArea()}
Local aDadosDM3  := {}
Local aCamposDM3 := {}
Local aDadosDM4  := {}
Local aCamposDM4 := {}
