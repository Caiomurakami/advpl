...
If lMotorista
	...
	If nOpc == 6
		...
		EndIf
	EndIf
	RegToMemory("DTO",nOpc # 2 .And. nOpc # 6)
...
Else
	...
	RegToMemory("DTU",nOpc # 2 .And. nOpc # 6)
	If nOpc == 2 .Or. nOpc == 3 .Or. nOpc == 6
		...
		If nOpcx == 3 // Entrada
			...
			If DTQ->DTQ_SERTMS == StrZero( 2, Len( DTQ->DTQ_SERTMS ) ) //-- Transporte
				...
				If	SF4->( ! MsSeek( xFilial('SF4') + cTes, .F. ) )
					Help( ' ', 1, 'TMSA20015',, " TES : "+ cTes,5,11)	 //"Tipo de entrada/saida informado no parametro MV_TESDR nao encontrado (SF4).
					A430ZerViag()
					Return( .F. )
				EndIf

				If	SF4->F4_TIPO != 'E'
					Help( ' ', 1, 'TMSA20016',, " TES : " + cTes,5,11)	//"Tipo de entrada/saida informado no parametro MV_TESDR nao eh um TES de entrada (SF4).
					A430ZerViag()
					Return( .F. )
				EndIf
...
LOCAL aArea    :=GetArea()
Local lVeiGen  := .F.
Local nQtdVei  := IIf(DTR->(ColumnPos('DTR_CODRB3'))> 0,3,2)
	...
	If ValType(aValid[nz,Len(aValid[nz])]) == "L" .And. !aValid[nz,Len(aValid[nz])] .And. !Empty(aValid[nz,nPosVei])
		...
		If lRet
			...
			If DA3->DA3_ATIVO == "2" //-- Nao
				...
				lRet := .F.
				Exit
			ElseIf nOpcx == 3 .And. DA3->DA3_STATUS == StrZero(3,Len(DA3->DA3_STATUS)) .And. DA3->DA3_FILVGA <> cFilOri .And. DA3->DA3_NUMVGA <> cViagem  // Em Viagem
				If nModulo <> 39 //-- Frete Embarcador
...
Local nOdoRb2		:= 0
Local nOdoRb3		:= 0
Local cAtivRDP		:= GetMv("MV_ATIVRDP",,"")
Local nPosLibSeg	:= 0
Local nPosValSeg	:= 0
...
If !IsInCallStack('TMSA146')
	...
	ElseIf Left( FunName(), 7 ) $ "TMSA140|TMSA141|TMSA143|TMSA144|" .Or. TmsVgeMod3()
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Fun‡„o    ³TA430MotOk³ Autor ³ Richard Anderson      ³ Data ³05.01.2007³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡„o ³ Verifica se a entrada do motorista pode ser realizada na   ³±±
±±³          ³ filial atual                                               ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Sintaxe   ³ TA430MotOk()                                               ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
