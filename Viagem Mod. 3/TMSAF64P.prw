...
If nOpc == 3 .Or. nOpc == 4
	...
	For nCount := 1 To oMdlDTR:Length()
		...
		If !oMdlDTR:IsDeleted()
			...
			If lRet
				lRet	:= VldPerAdi( cCodVei, nPerAdi )
			EndIf
			...
			If lRet
				lRet    := VldPamSDG(oModel, cCodOpe, @nVlrAdto )  
			EndIf
...
*/
//-------------------------------------------------------------------
Static Function VldQtdDTR( nQtdDTR )
Local lRet		:= .T. 
...
*/
//-------------------------------------------------------------------
Static Function VldPerAdi( cCodVei , nPerAdi )
Local lRet			:= .T. 
...
*/
//-------------------------------------------------------------------
Static Function VldContr(cCodVei)
Local lRet			:= .T. 
Local aContrFor		:= {}
Local aLojForDA3	:= {}
Local cCodForn		:= ""
...
If Len(aLojForDA3) > 0 .And. !Empty(aLojForDA3[1]) .And. !Empty(aLojForDA3[2])
	...
	ElseIf Len(aContrFor) > 0 .And. (Empty(aContrFor[1][13]) .Or. aContrFor[1][13] == '0') 
	...
	For nAux := 1 To oMdlDLD:Length()
		...
		If !oMdlDLD:IsDeleted()
			...
			If nVlrPdg	> 0 				
				...
				lRet	:= VldFormPg(cCodVei,lCartao)

				If lRet
					lRet	:= VldIdOpe(cCodVei,cIdOpe)
				EndIf
			EndIf
...
*/
//-------------------------------------------------------------------
Static Function VldFormPg(cCodVei,lCartao)
Local lRet		:= .T. 
Local lLotacao	:= .F. 
...
*/
//-------------------------------------------------------------------
Static Function VldIdOpe(cCodVei,cIdOpe)
Local lRet		:= .T. 
Local lLotacao	:= .F. 
...
*/
//-------------------------------------------------------------------
Static Function VldPamSDG( oModel, cCodOpe, nVlrAdto )
Local aArea		:= GetArea()
...
If lRet
	If AllTrim(FwFldGet("DLD_TIPPAR")) == '3' .And.  AllTrim(FwFldGet("DLD_FORPAG")) <> '1'  
		Help( ' ', 1, 'TMSA240A4') //A forma de pagamento da parcela de pedagio deve ser Cartão.
		lRet:= .F.
	EndIf
EndIf
