...
Local cProgCall  := ''
Local aRentab	 := {}
Local lRotaInt   := SuperGetMV("MV_ROTAINT",,.F.)  //Integração Rota Inteligente
Local aRotaInt   := {}
Local aFreteBr   := {} 
Local nAux		 := 1 
Local aStatus	 := {}
...
If lRotaInt .And. TableInDic('DLU') .And. ExistFunc('A144RotInt')
	AAdd(aRotaInt, { STR0166 ,'A144RotInt',0,3,0 ,.F. })     //-- Integra Roteirizador
	AAdd(aRotaInt, { STR0167 ,'A144RotInt',0,6,0,.F. })   	//-- Consulta Roteirizador
	AAdd(aRotaInt, { STR0079 ,'TMSA046Leg',0,6,0,.F. })   	//-- Legenda Roteirizador
	AAdd(aRotina, { STR0165   , aRotaInt ,0,6,0,.F. }) 	    //-- Roteirizador
EndIf
...
Else
	...
	If  lRotaInt
		oBrw144:AddStatusColumns( {||TMSMAPSTAT( DTQ->DTQ_FILORI , DTQ->DTQ_VIAGEM )}, {||TMSA046Leg()} )
	EndIf
	oBrw144:Activate()
...
//-- ROTA INTELIGENTE
//-------------------------------------------------------------------------
If lRet .And. (nOpcx == 4 .Or. nOpcx == 3 ) .And. lRotaInt
	
	oRotaInt    := TMSBCARotaInt():New(,,"POST", M->DTQ_FILORI , M->DTQ_VIAGEM , !IsBlind() )
	If oRotaInt:GetTypeCallBack() == "1" //-- Envia callback
		oRotaInt:Token()
		oRotaInt:PlanningPost()
	EndIf
	oRotaInt:Destroy()
...
Chamada para o Registros de Ocorrencias
...
If cSerTms $ "23"
	...
	AAdd( aRotina, { STR0013, "TMSA144Fbr"  ,0 ,1,0,Nil}) //"Status"
...
Else
	...
	aRotina	:= {	{ STR0008, "AXPesqui"   ,0 ,1 ,0,.F.},; //"Pesquisar"
					...
					{ STR0013, 'TMSA144Fbr'	,0 ,1 ,0, Nil} } //"Status"
EndIf
...
@type function
/*/
Function A144RotInt( cTab ,nRecno ,nMenu )
...
Local cUrlTotvs	:= "https://mirror.totvs.store/br/produto/publicacao-automatica-de-cargas.html"
...
If nRecno > 0 .And. DTQ->DTQ_TIPVIA == '3' //-- Viagens planejadas
	...
	If TMSAFretBr( @lTrial, @nQtdFrete )
		lRet	:= .T.

		If lTrial .And. nQtdFrete > 30
			cTit	:= STR0178 //-- "Automação Automática de Cargas – Portal Fretebras"
			cMsg	:= STR0183  + chr(10) + chr(13) //-- "Gostando do recurso de publicação automática de cargas no portal Fretebras?"  
			cMsg	+= STR0184  + cValToChar(nQtdFrete) + STR0185 //"Notamos que você já utilizou " + cValToChar(nQtdFrete) +  " movimentações do total de 50 disponíveis gratuitamente." + chr(10) + chr(13)  
			cMsg	+= STR0186  + chr(10) + chr(13)//"Programe agora a contratação desta oferta para continuar aproveitando os benefícios deste recurso." + chr(10) + chr(13) 	
			cMsg	+= STR0182 //-- "Quer saber mais?"
	...
	Else 
		If lTrial
			cTit	:= STR0178 //-- "Automação Automática de Cargas – Portal Fretebras"
			cMsg	:= STR0187  + chr(10) + chr(13)   //-- "Você utilizou todas as movimentações gratuitas disponíveis para avaliação deste recurso." + chr(10) + chr(13)  
			cMsg	+= STR0188  + chr(10) + chr(13)  //-- "Para continuar aproveitando seus benefícios, contrate agora!" + chr(10) + chr(13) 	//-- "Conheça todos os benefícios e realize a ativação por meio da TOTVS Store." 
			cMsg	+= STR0182 //-- "Quer saber mais?"
		...
		Else
			cTit	:= STR0178 //-- "Automação Automática de Cargas – Portal Fretebras"  
			cMsg	:= STR0179 + chr(10) + chr(13)  //-- "Você possui 50 movimentações gratuitas para conhecer e avaliar este recurso. Para tanto, basta informar as credenciais de acesso do portal Fretebras. Vamos lá?"    
			cMsg	+= STR0180 + chr(10) + chr(13)  //-- "Se deseja saber mais sobre esta oferta, acesse aTotvs Store"
			cMsg	+= STR0182 //-- "Quer saber mais?"
